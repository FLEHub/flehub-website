...
export async function POST(request: Request) {
  ...
  try {
    ...
    if (action === 'createStudent' || action === 'createSchoolStudent') {
      ...
      const cefr_level = payload.cefr_level ? String(payload.cefr_level) : null;
      const exam_session_id = payload.exam_session_id ? String(payload.exam_session_id) : null;
      ...
      const { data: student, error } = await supabase
        .from('school_students')
        .insert({ school_id: school.id, first_name, last_name })
        .select(SCHOOL_STUDENT_COLUMNS)
        .maybeSingle();
      if (error) throw error;

      // Si un niveau ET une session ont été choisis, on crée aussi l'inscription à l'examen.
      if (student && cefr_level && exam_session_id) {
    ...
    if (action === 'updateSchoolStudent') {
      ...
      if (!student_id) return jsonError('Élève introuvable.');
      if (!first_name || !last_name) return jsonError('Prénom et nom sont requis.');
      ...
      if (cefr_level && exam_session_id) {
        if (existingEnrollment) {
          const { error: enrollUpdateError } = await supabase
            .from('student_enrollments')
            .update({ cefr_level, exam_session_id })
            .eq('id', existingEnrollment.id);
          if (enrollUpdateError) throw enrollUpdateError;
        } else {
          const { error: enrollInsertError } = await supabase.from('student_enrollments').insert({
            student_id,
            exam_session_id,
            cefr_level,
            active: true,
