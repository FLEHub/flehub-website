...
export default function AdminExamPapersPage() {
  ...
  return (
    <div className="p-6 space-y-6">
      ...
      <Card>
        <CardHeader><CardTitle>Téléverser un sujet</CardTitle></CardHeader>
        <CardContent className="grid grid-cols-1 md:grid-cols-4 gap-3 items-end">
          ...
          <Button onClick={upload} disabled={!sessionId || !file || uploading}
              ...
              <CardContent className="space-y-2">
                {COMPETENCIES.map(([key, label]) => (
                  <div key={key} className="flex items-center justify-between rounded-lg border p-3">
                    ...
                    <Badge variant={papers.some((p) => p.competency === key) ? 'default' : 'secondary'}>
                      {papers.some((p) => p.competency === key) ? 'Disponible' : 'Manquant'}
                    </Badge>
                  </div>
                ))}
              </CardContent>
