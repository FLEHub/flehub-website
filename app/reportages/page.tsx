...
import { ReportageCardLink } from '@/components/portal/reportage-card-link'
...
export default async function ReportagesPage({ searchParams }: Props) {
      ...
      <main className="flex-1 max-w-5xl w-full mx-auto px-6 py-10 sm:py-14">
        <div className="mb-8 space-y-2">
          ...
          <p className="text-gray-600 max-w-2xl">
            Les derniers reportages audio publiés par {shortName}.
          </p>
        </div>
        ...
        <div className="flex flex-wrap gap-2 mb-8">
          ...
          {categories.map((c) => (
            <Link
              key={c.id}
              href={`/reportages?categorie=${c.slug}`}
              className={
                categorySlug === c.slug
                  ? 'text-sm font-medium px-3 py-1.5 rounded-lg bg-[#1E5FA8] text-white'
                  : 'text-sm font-medium px-3 py-1.5 rounded-lg bg-white border border-gray-200 text-gray-600 hover:border-[#1E5FA8] hover:text-[#1E5FA8] transition-colors'
        ...
        {reportages.length === 0 ? (
          <p className="text-sm text-gray-500 py-12 text-center">
            {categorySlug
              ? 'Aucun reportage publié dans cette catégorie.'
          ...
          <div className="bg-white/80 rounded-2xl border border-gray-100 px-5 sm:px-8 py-2">
            {reportages.map((reportage) => (
              <ReportageCardLink key={reportage.id} reportage={reportage} />
            ))}
          </div>
        )}
      </main>
