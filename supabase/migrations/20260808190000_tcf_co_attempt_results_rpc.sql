/*
  # RPC résultats TCF CO (après tentative)

  get_student_co_attempt_results(attempt_id) :
  - accessible uniquement au propriétaire (ou admin)
  - recalcule le score, l'enregistre dans student_co_attempts.score
  - renvoie le détail question par question (réponse donnée + bonne réponse)
    uniquement pour une tentative terminée (ou la clôture au passage)
*/

CREATE OR REPLACE FUNCTION public.get_student_co_attempt_results(
  p_attempt_id uuid
)
RETURNS jsonb
LANGUAGE plpgsql
...
REVOKE ALL ON FUNCTION public.get_student_co_attempt_results(uuid) FROM PUBLIC;
GRANT EXECUTE ON FUNCTION public.get_student_co_attempt_results(uuid) TO authenticated;
