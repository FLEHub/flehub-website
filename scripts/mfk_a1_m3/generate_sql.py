#!/usr/bin/env python3
"""Build the idempotent SQL seed for MFK A1 Module 3 from content.py."""
...
OUT = ROOT / "supabase/migrations/20260823190000_elearning_mfk_a1_module3_orienter_installer.sql"
...
def main() -> None:
    lines = [HELPERS, BOOT % (sql_str(MODULE["title"]), sql_str(MODULE["description"]), sql_str(MODULE["description"]))]

    for seq_index, seq in enumerate(SEQUENCES):
        lines.append(f"  -- ===== {seq['title']} =====")
        lines.append("  SELECT s.id INTO v_seq_id")
        lines.append("  FROM elearning_sequences s")
        lines.append("  WHERE s.module_id = v_module_id AND s.title = " + sql_str(seq["title"]))
