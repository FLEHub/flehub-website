...
type Props = {
  reportage: ReportageCardData
}

export function ReportageCardLink({ reportage }: Props) {
  return (
    <Link
      ...
      <div className="flex flex-col sm:flex-row gap-4 sm:gap-6">
        ...
        <div className="min-w-0 flex-1 space-y-1.5">
          <div className="flex flex-wrap items-center gap-x-2 gap-y-1 text-xs text-gray-500">
            {reportage.category_name && (
              <span className="text-[#00A550] font-medium">
                {reportage.category_name}
              </span>
            )}
            {reportage.published_at && (
              <span>{formatArticleDate(reportage.published_at)}</span>
            )}
          </div>
